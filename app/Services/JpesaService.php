...
<g7bill>
    ...
    private function handleVoucherPayment($transaction)
    {
        ...
        // CRITICAL FIX: Mark voucher as used immediately when payment is completed
        // This prevents duplicate voucher usage regardless of SMS success/failure
        ...
        if ($voucher && $voucher->status === "unused") {
            ...
            Log::info("JpesaService: Voucher marked as used immediately", [
                "transaction_id" => $transaction->transaction_id,
            ...
            try {
                ...
                Log::error("JpesaService: Exception during SMS sending (voucher already marked as used)", [
                    "transaction_id" => $transaction->transaction_id,
                    "voucher_code" => $voucher->code,
